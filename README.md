...
**Settings**: `max_concurrent` (default 2 for api, 1 for a subscription: a personal session was never provisioned for parallel automation); `thinking` (`true` follows the job's level, `false` is off, or a fixed `low`/`medium`/`high`); `context_window` to override OpenClaw's catalog for a model newer than it knows (otherwise the catalog value is used, with a safety margin). Api and subscription jobs get their own concurrency ceiling (`NOMARMY_MAX_POOL_WORKERS`, default 4), additive to local workers; raise one agent's with `nomarmy agents update <name> --max-concurrent <n>`. It counts across every nomArmy session on the machine.
