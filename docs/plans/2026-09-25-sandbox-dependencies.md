...
7. **`services`: fake services on a private network.** Podman `--internal` network, service containers declared by a harness, health-checked before the profile runs. First: a mock OIDC provider. *Medium.*
...
9. **`allowlist` network for verification.** The egress proxy, per-host opt-in, host-provided test credentials, the record line. *High; security review before merging.*
