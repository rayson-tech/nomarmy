...
function fixture(t, files = {}) {
  ...
  const local = (registries) => fs.writeFileSync(path.join(repo, ".nomarmy.local.yml"), YAML.stringify({ registries }));
...
test("registry declarations are local-only, validated and excluded from the loaded job contract", (t) => {
  ...
  for (const name of [".nomarmy.yml", ".nomarmy.yaml"]) {
    ...
    assert.throws(() => composeSandboxImage(f.repo), { message: expected });
...
test("registry paths reject inline values, malformed declarations, missing files and directories without echoing input", (t) => {
  ...
  assert.throws(() => composeSandboxImage(f.repo), { message: "registries: cannot parse .nomarmy.local.yml" });
  f.local({ npm: "../credential file" });
  assert.match(composeSandboxImage(f.repo).dockerfile, /--mount=type=secret,id=npm,/);
});
...
test("registry Go and cargo use exact mounts and private patterns only during download", (t) => {
  ...
  const recipe = composeSandboxImage(f.repo).dockerfile;
...
test("registry tag hashes declarations and rotation without exposing secret bytes", (t) => {
  const f = fixture(t, npmFiles);
  f.local({ npm: f.secret });
  const first = composeSandboxImage(f.repo);
  ...
  assert.equal(composeSandboxImage(f.repo).image, first.image);
  fs.writeFileSync(f.secret, TOKEN + "-rotated");
  const rotated = composeSandboxImage(f.repo);
  ...
  f.local({ npm: other });
  assert.notEqual(composeSandboxImage(f.repo).image, rotated.image);
  f.local({ npm: f.secret });
