...
// see lib/sandbox-images.mjs's pythonRequirementsFor(). Paths are relative to
// the repository root; this schema only validates shape, never touches the
// filesystem (no "does this file exist" check belongs here).
...
export const environmentSchema = z
  .object({
    ...
    python: pythonEnvironmentSchema.optional(),
    // lib/sandbox-images.mjs: install: false turns off the npm lockfile installs.
    node: z.object({ install: z.boolean().optional() }).strict().optional(),
  })
  .strict();
...
// What the repo requires of every implement job, whatever the General
// decides per job. This file is read from the operator's checkout, never a
// job's worktree, so neither the General nor a worker can relax it.
//   require_verification     a job needs a verification profile, and nomArmy
//                            commits only when that verification passed
//   require_regression_check verify_regression can't be switched off
export const policySchema = z
  .object({
    require_verification: z.boolean().optional(),
    require_regression_check: z.boolean().optional(),
  })
  .strict();
...
export const configSchema = z
  .object({
    ...
    // Who the General calls for what in this repo. Owned by lib/army.mjs,
