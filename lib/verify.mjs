...
import { resolveSandboxImage } from "./sandbox-images.mjs";
...
export function createPodmanExecutor({ podman = "podman" } = {}) {
  return {
    ...
    async run({ command, cwd, image, jobId, network, user, workdir, timeoutMs, maxOutputBytes, nodeModulesSource }) {
      const args = buildPodmanArgs({ image, cwd, command, jobId, network, user, workdir, nodeModulesSource });
  ...
  return async function runVerification(context = {}) {
    ...
    try {
      sandboxImage = resolveSandboxImage({
        cwd, explicitImage, defaultImage: DEFAULT_AGENT_IMAGE,
        ...(sandboxImageRun ? { run: sandboxImageRun } : {}),
      });
    } catch (error) {
      // A lazy Go/Rust image build failed (offline, disk full, apt/rustup
      // error). Running the job's real commands against the wrong image
      // would look like a genuine test failure rather than the
