...
export function createPodmanExecutor({ podman = "podman", collect = spawnCollect, healthTimeoutMs = 30_000, healthIntervalMs = 250 } = {}) {
  return {
    ...
    async startServices({ services, image, jobId }) {
...
export function createVerificationRunner(options = {}) {
  ...
  return async function runVerification(context = {}) {
    ...
    if (specs.some((spec) => spec.network === "allowlist")) return notRun("allowlist harnesses require operator-local provisioning", "environment-not-provisioned");
    ...
    let serviceRun;
    try {
      if (services.length) serviceRun = await executor.startServices({ services, image: sandboxImage, jobId });
    } catch (error) {
      return notRun(`service setup failed: ${error.message}`, "services-unavailable");
    }
    try {
      for (const command of commands) {
        ...
        try {
          raw = await executor.run({
            ...
            maxOutputBytes,
            nodeModulesSource,
            env: { ...harnessEnv, ...verificationEnv },
            shmMb,
          });
        } catch (error) {
      ...
      if (serviceRun) {
        ...
        catch (error) { return notRun(`service cleanup failed: ${error.message}`, "services-cleanup-failed"); }
      }
    }
