...
export function createPodmanExecutor({ podman = "podman", collect = spawnCollect, healthTimeoutMs = 30_000, healthIntervalMs = 250 } = {}) {
  return {
    ...
    async startServices({ services, image, jobId, allow = [] }) {
      ...
      const call = (args, timeoutMs = 20_000) => collect(podman, args, { timeoutMs, maxOutputBytes: 4096 });
      ...
      try {
        ...
        return { network, cleanup, ...(allow.length ? { logs: async () => {
          ...
          return result.stdout.split("\n").filter(line => /^[a-z0-9.-]+:[0-9]+ (allowed|denied)$/.test(line)).join("\n");
        } } : {}) };
      } catch (error) {
    ...
    async run({ command, cwd, image, jobId, network, user, workdir, timeoutMs, maxOutputBytes, nodeModulesSource, env, shmMb }) {
      const args = buildPodmanArgs({ image, cwd, command, jobId, network, user, workdir, nodeModulesSource, env, shmMb });
      const started = Date.now();
      const outcome = await collect(podman, args, { timeoutMs, maxOutputBytes });
...
export function createVerificationRunner(options = {}) {
  ...
  return async function runVerification(context = {}) {
    ...
    const networkRecord = networkPolicy ? { allowlist: networkPolicy.allow, credentials: Object.keys(networkPolicy.env) } : null;
    ...
    let proxyLog = "";
    try {
      if (services.length || networkPolicy) serviceRun = await executor.startServices({ services, image: sandboxImage, jobId, ...(networkPolicy ? { allow: networkPolicy.allow } : {}) });
    } catch (error) {
    ...
    const proxyEnv = networkPolicy ? Object.fromEntries([
      ...["HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy"].map(name => [name, "http://egress:3128"]),
    ...
    try {
      for (const command of commands) {
        ...
        const stdout = capOutput(redact(raw?.stdout), maxOutputBytes);
        const stderr = capOutput(redact(raw?.stderr), maxOutputBytes);
        const entry = {
          command: redact(command),
          started: raw?.started !== false,
          timedOut: Boolean(raw?.timedOut),
          exitCode: Number.isInteger(raw?.exitCode) ? raw.exitCode : null,
          reason: raw?.reason == null ? null : redact(raw.reason),
      ...
      if (serviceRun) {
        let logError;
        try { if (serviceRun.logs) proxyLog = redact(await serviceRun.logs()); }
        catch { logError = true; }
        try { await serviceRun.cleanup(); }
        catch (error) { return notRun(redact(`service cleanup failed: ${error.message}`), "services-cleanup-failed", { ...networkInfo, output: proxyLog }); }
        if (logError) return notRun("could not capture egress verdict log", "egress-log-failed", networkInfo);
      }
    }
    ...
    if (verdict.status === "not_run") {
      return notRun(verdict.detail, "sandbox-unavailable", { ...networkInfo, output: proxyLog });
    }
    ...
    return {
      ...
      output: (proxyLog ? `${proxyLog}\n\n` : "") + results.map((r) => `$ ${r.command}  (exit ${r.exitCode ?? "none"}${r.timedOut ? ", timed out" : ""})\n${r.stdout}${r.stderr ? `\n[stderr]\n${r.stderr}` : ""}`).join("\n\n"),
    };
  };
}
