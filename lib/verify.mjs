...
//
// Therefore commands only ever execute inside the Docker sandbox image, as a
// non-root user, with `--network none`. If Docker is missing, the image is
...
/** Matches the non-root `sandbox` user baked into `docker/Dockerfile`. */
 ...
 * @returns {string[]} argv for `docker`
 */
...
// ---------------------------------------------------------------------------
// the real Docker executor (the only place a host process is spawned)
// ---------------------------------------------------------------------------
...
/**
 * Executor backed by the real Docker CLI. Tests inject a fake in its place.
 * @param {{ docker?: string }} [options]
 */
export function createDockerExecutor({ docker = "docker" } = {}) {
  return {
    /** Is a usable sandbox present? Never throws. */
    async probe({ image }) {
      const version = await spawnCollect(docker, ["version", "--format", "{{.Server.Version}}"], {
        timeoutMs: 20_000,
        maxOutputBytes: 4096,
      });
      if (!version.spawned) {
        return { available: false, reason: `docker CLI is not available: ${version.stderr.trim() || "spawn failed"}` };
      }
      if (version.timedOut) {
        return { available: false, reason: "docker did not respond within 20s" };
      }
      if (version.code !== 0) {
        return {
          available: false,
          reason: `docker daemon is not reachable: ${version.stderr.trim() || `exit ${version.code}`}`,
        };
      }

      const inspect = await spawnCollect(docker, ["image", "inspect", image], {
        timeoutMs: 20_000,
        maxOutputBytes: 4096,
      });
    ...
    async run({ command, cwd, image, jobId, network, user, workdir, timeoutMs, maxOutputBytes }) {
      const args = buildDockerArgs({ image, cwd, command, jobId, network, user, workdir });
      const started = Date.now();
      const outcome = await spawnCollect(docker, args, { timeoutMs, maxOutputBytes });
      const durationMs = Date.now() - started;

      if (!outcome.spawned) {
        return {
          started: false,
          reason: `docker could not be executed: ${outcome.stderr.trim() || "spawn failed"}`,
          stdout: "",
      ...
      // 125 is docker's own "the run itself failed" code: the container never
      // started, so this is absence of evidence rather than a failing test.
      if (outcome.code === 125) {
        return {
          started: false,
          reason: `container failed to start: ${outcome.stderr.trim() || "docker exit 125"}`,
          stdout: outcome.stdout,
          stderr: outcome.stderr,
          durationMs,
        };
...
export function createVerificationRunner(options = {}) {
  ...
  return async function runVerification(context = {}) {
    ...
    if (!probe || probe.available !== true) {
      return notRun(
        `${probe?.reason || "the Docker sandbox is unavailable"}; verification commands come from the repository and are never executed on the host`,
        "sandbox-unavailable",
      );
    }
