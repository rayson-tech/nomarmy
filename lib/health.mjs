...
/**
 * Models that answered "Unknown model" on a real job in the last day,
 ...
 */
export function unknownModelIssues(jobRecords, { now = Date.now() } = {}) {
  const failed = new Map();
  for (const m of jobRecords) {
    if (now - Date.parse(m.finishedAt ?? "") > DAY) continue;
    const hit = /Unknown model: ([\w.-]+\/[\w.:-]+)/.exec(`${m.workerError ?? ""} ${m.worker?.error ?? ""}`);
    const model = hit?.[1].replace(/[.:]+$/, ""); // the sentence's own trailing period isn't part of the name
    if (model) failed.set(model, (failed.get(model) ?? 0) + 1);
  }
  return [...failed].map(([model, n]) => ({ id: `unknown-model:${model}`, severity: "warn",
    title: `${model} answered "Unknown model" on ${n} job${n === 1 ? "" : "s"} today`,
    detail: "OpenClaw can't run it right now (it may still be listed in the catalog), so every role on it fails.",
...
export async function runHealthChecks({ now = Date.now(), openclawCmd = process.env.NOMARMY_OPENCLAW_CMD || "openclaw", run = runBounded, armySummary = null, agentsError = null, jobsRoot = null, pidAlive = () => true } = {}) {
  ...
  if (jobsRoot) {
    ...
    issues.push(...unknownModelIssues(records, { now }));
