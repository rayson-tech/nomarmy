...
export async function runHealthChecks({ now = Date.now(), openclawCmd = process.env.NOMARMY_OPENCLAW_CMD || "openclaw", run = runBounded, armySummary = null, agentsError = null, jobsRoot = null, pidAlive = () => true, agents = null, openclawConfig = null, vendors = {}, modelsInUse = null, autoPruned = null } = {}) {
  const issues = [];
  if (autoPruned?.pruned) issues.push({ id: `auto-prune:${new Date(now).toISOString()}`, severity: "info",
    title: `Freed ${(autoPruned.freedBytes / 1024 ** 3).toFixed(2)} GB: runtime data of ${autoPruned.pruned} finished job${autoPruned.pruned === 1 ? "" : "s"} older than ${autoPruned.olderThanHours}h`,
