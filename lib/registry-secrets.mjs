...
export function registryInstallRun(command, ecosystem, secrets) {
  ...
  const mounts = [
    ...
    "--mount=type=tmpfs,target=/home/node/.cache",
  ...
  const env = ecosystem === "go"
    ...
    : ecosystem === "npm" ? "export npm_config_cache=/tmp/npm-cache COREPACK_HOME=/tmp/corepack YARN_CACHE_FOLDER=/tmp/yarn-cache YARN_GLOBAL_FOLDER=/tmp/yarn-global BUN_INSTALL_CACHE_DIR=/tmp/bun-cache; " : "";
  ...
  return `RUN ${mounts} { : ${salt}; ${env}${command}; } >/dev/null 2>&1`;
}
