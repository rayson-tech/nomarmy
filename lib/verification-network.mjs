...
export function validateVerificationNetwork(value) {
  ...
  if (!mapping(value) || Object.keys(value).some(k => !['allow', 'env', 'keep_output'].includes(k)) ||
  ...
  if (value.keep_output !== undefined && typeof value.keep_output !== 'boolean') throw new Error('verification_network.keep_output must be boolean');
  let allow;
  ...
  return { allow, env, ...(value.keep_output !== undefined ? { keep_output: value.keep_output } : {}) };
}
...
export function loadVerificationNetwork(projectDir, { check = (args) => spawnSync('git', args, { cwd: projectDir, encoding: 'utf8', timeout: 5000 }) } = {}) {
  ...
  let parsed;
  try { parsed = stat.isFile() && !stat.isSymbolicLink() ? parseYaml(fs.readFileSync(file, 'utf8')) : null; }
  catch { throw new Error('invalid operator-local YAML'); }
  // The file also holds other operator-local settings (registries, the army);
  // provenance only matters when it grants network.
  if (parsed?.verification_network === undefined && stat.isFile() && !stat.isSymbolicLink()) return null;
  ...
  return validateVerificationNetwork(parsed.verification_network);
}

export function credentialForms(secrets) {
  return [...new Set(secrets.filter(Boolean).flatMap(secret => {
    const bytes = Buffer.from(secret);
    const percent = [...bytes].map(b => '%' + b.toString(16).padStart(2, '0')).join('');
    return [secret, bytes.toString('base64'), bytes.toString('base64url'),
      bytes.toString('hex'), bytes.toString('hex').toUpperCase(),
      encodeURIComponent(secret), encodeURIComponent(secret).replace(/%[0-9A-F]{2}/g, s => s.toLowerCase()),
      percent, percent.toUpperCase()];
  }))].sort((a, b) => b.length - a.length);
}

export function redactCredentials(value, secrets, { truncated = true } = {}) {
  let text = String(value ?? '');
  for (const secret of credentialForms(secrets)) {
    text = text.split(secret).join('***');
    // Capturing may have cut the final chunk in the middle of a credential.
    for (let n = truncated ? Math.min(secret.length - 1, text.length) : 0; n > 0; n--) {
