...
/**
 * What the General asked for that it didn't get, as plain sentences: the
 * configured ceiling (or the default, when none is configured) always
 * wins. A $15 request silently becoming $10 read as a bug.
 */
export function describeLoweredLimits(configured = {}, requested = {}, resolved) {
  const label = { max_jobs: "jobs", max_api_usd: "api spend ($)", max_hours: "hours" };
  return LIMIT_KEYS.filter((k) => Number.isFinite(requested?.[k]) && requested[k] > resolved[k]).map((k) => {
    const source = configured?.[k] !== undefined ? "your configured run_limits" : "the default (no run_limits configured)";
    return `${label[k]} capped at ${resolved[k]}, not ${requested[k]}: that's the ceiling from ${source}. Raise it with army.run_limits.${k} in ~/.config/nomarmy/config.yml or .nomarmy.local.yml.`;
  });
}
...
export function runTotals(run, { now = Date.now() } = {}) {
  ...
  const label = { max_jobs: "jobs", max_api_usd: "api spend ($)", max_hours: "hours" };
  // Past a limit is the loudest case, not a silent one: warnings used to
  // stop at 100%, so a run at 8.46 of its 4 hours reported nothing.
  const warnings = [
    ...LIMIT_KEYS.filter((k) => share[k] >= 1).map((k) => `run is OVER its ${label[k]} limit (${used[k]} of ${run.limits[k]}); no more jobs are admitted`),
    ...LIMIT_KEYS.filter((k) => share[k] >= run.limits.warn_at && share[k] < 1).map((k) => `run is at ${Math.round(share[k] * 100)}% of its ${label[k]} limit (${used[k]} of ${run.limits[k]})`),
  ];
 ...
 */
export function runAdmissionProblems(run, { agentName = "local", now = Date.now(), running = 0 } = {}) {
  ...
  // Jobs still running count too: a job is recorded into its run only when
  // it finishes, so jobs started together could otherwise overshoot.
  if (used.max_jobs + running >= run.limits.max_jobs) problems.push(`run "${run.id}" has used all ${run.limits.max_jobs} of its jobs${running ? ` (${running} still running)` : ""}`);
