 ...
 * @returns {object}
 */
export function resolveMemoryPool(hardware) {
  const hw = hardware || {};
  const mem = hw.memory || {};
  const gpu = hw.gpu || {};
  ...
  if (systemAvailable || systemTotal) {
    return {
      kind: "system",
      coherent: false,
      poolBytes: systemAvailable || systemTotal,
...
function capacityFor(pool, model, contextPerNom, bytesPerElement) {
  ...
  const budget =
    ...
    model.weightsBytes;
  const perSlot = kvPerSlot + R.computeBufferPerSlotBytes + R.dockerSandboxPerNomBytes;
...
function memoryBreakdown(pool, model, noms, contextPerNom, bytesPerElement) {
  ...
  const sandboxes = R.dockerSandboxPerNomBytes * noms;

  const gpuResidentBytes = model.weightsBytes + kvTotal + computeBuffers + R.runtimeOverheadBytes;
  const systemResidentBytes = sandboxes + R.osBytes + R.coordinatorBytes;
...
function assumptionsFor(model, pool, bytesPerElement) {
  const list = [
    ...
    `Headroom reserved: ${formatBytes(RESERVES.osBytes)} OS, ${formatBytes(RESERVES.coordinatorBytes)} coordinator, ` +
      `${formatBytes(RESERVES.dockerSandboxPerNomBytes)} Docker sandbox per nom, ` +
