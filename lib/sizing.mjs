...
export const RESERVES = Object.freeze({
  ...
  /** The Docker sandbox container nomArmy starts for every job. */
  dockerSandboxPerNomBytes: Math.round(1.5 * GIB),
  ...
  safetyFraction: 0.9,
  /** Share of Apple unified memory the GPU may hold wired. */
  unifiedGpuFraction: 0.75,
  /**
...
export function kvBytesPerSlot(model, contextPerNom, bytesPerElement = DEFAULT_BYTES_PER_KV_ELEMENT) {
  return (
    model.layers * model.kvHeads * (model.keyLength + model.valueLength) * contextPerNom * bytesPerElement
  );
}
 ...
 *
 * kinds:
 *   "vram"    discrete NVIDIA VRAM; Docker/OS/coordinator live in system RAM
 ...
 */
/**
 * Memory Docker has carved out of host RAM and that a local model can never use.
 *
 * On Docker Desktop (Windows, macOS) containers run inside a Linux VM with a
 * fixed memory allocation, reserved whether or not anything is running. That
 ...
 */
export function dockerVmReservation(hardware) {
  const hw = hardware || {};
  const total = Number(hw.docker && hw.docker.totalMemoryBytes) || 0;
  if (!total) return 0;
...
export function resolveMemoryPool(hardware, options = {}) {
  // Docker Desktop holds its VM allocation out of host RAM permanently, so the
  // pool a local model can actually use is smaller than total RAM suggests.
  const dockerVmBytes = dockerVmReservation(hardware);
  const hw = hardware || {};
  const mem = hw.memory || {};
  const gpu = hw.gpu || {};
...
function capacityFor(pool, model, contextPerNom, bytesPerElement) {
  ...
  if (pool.kind === "vram") {
    ...
    if (sysPool) {
      systemBudget = sysPool * R.safetyFraction - R.osBytes - R.coordinatorBytes;
      bySystem = Math.floor(systemBudget / R.dockerSandboxPerNomBytes);
    }
    return {
  ...
  // Must mirror memoryBreakdown exactly: a Docker Desktop VM allocation comes
  // off the pool, and its sandboxes are then inside the VM rather than extra.
  const dockerVm = pool.dockerVmBytes ?? 0;
  const budget =
    ...
    model.weightsBytes;
  const perSlot =
    kvPerSlot + R.computeBufferPerSlotBytes + (dockerVm > 0 ? 0 : R.dockerSandboxPerNomBytes);
...
function memoryBreakdown(pool, model, noms, contextPerNom, bytesPerElement) {
  ...
  // On a Docker Desktop host the sandboxes live inside the VM, so charging them
  // per nom on top of the whole VM allocation would double-count.
  const dockerVmBytes = pool.dockerVmBytes ?? 0;
  const sandboxes = dockerVmBytes > 0 ? 0 : R.dockerSandboxPerNomBytes * noms;

  const gpuResidentBytes = model.weightsBytes + kvTotal + computeBuffers + R.runtimeOverheadBytes;
  const systemResidentBytes = sandboxes + dockerVmBytes + R.osBytes + R.coordinatorBytes;
  ...
  return {
    ...
    reserved: {
      ...
      dockerVmBytes,
      dockerSandboxBytes: sandboxes,
      dockerSandboxPerNomBytes: R.dockerSandboxPerNomBytes,
...
function environmentWarnings(hardware, pool, model) {
      ...
      warning(
        ...
        pool.coherent && pool.gpuCount > 0
          ? "GPU memory and system RAM are the same physical pool on this machine (coherent memory), " +
              ...
              "every Docker sandbox all draw on the one pool."
          : pool.appleUnified
            ...
            : "This machine has coherent unified memory: CPU and GPU share one physical pool, so there is no " +
                ...
                "Docker sandbox all draw on the one pool.",
      ),
    );
  }
  ...
  if (hw.docker && hw.docker.available === false) {
    out.push(
      warning(
        "docker_unavailable",
        "warning",
        "Docker did not respond. nomArmy starts a sandbox container per job, so worker jobs will not run " +
          "until the daemon is available; its memory is still reserved in this plan.",
      ),
    );
  }
...
function assumptionsFor(model, pool, bytesPerElement) {
  const list = [
    ...
    `Headroom reserved: ${formatBytes(RESERVES.osBytes)} OS, ${formatBytes(RESERVES.coordinatorBytes)} coordinator, ` +
      (pool.dockerVmBytes ? `${formatBytes(pool.dockerVmBytes)} for the Docker Desktop VM (half its reported ceiling, a heuristic ` +
        `for steady-state pressure -- sandboxes run inside it and are not charged again), ` : `${formatBytes(RESERVES.dockerSandboxPerNomBytes)} Docker sandbox per nom, `) +
...
export function recommend(input = {}) {
  ...
  if (isCloudExecution(execution)) {
    return cloudRecommendation(execution, target);
  }
  ...
  const capacity = capacityFor(pool, model, target, bytesPerKvElement);
...
export default { recommend, evaluateConfig, RESERVES, DEFAULT_TARGET_CONTEXT_PER_NOM };
