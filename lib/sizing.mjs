...
export function bytesPerKvElementForCacheTypes(cacheTypeK, cacheTypeV) {
  ...
  const k = resolve(cacheTypeK);
  const v = resolve(cacheTypeV);
  if (k === undefined && v === undefined) return DEFAULT_BYTES_PER_KV_ELEMENT;
...
export default { recommend, evaluateConfig, RESERVES, DEFAULT_TARGET_CONTEXT_PER_NOM, KV_CACHE_TYPE_BYTES, bytesPerKvElementForCacheTypes };
