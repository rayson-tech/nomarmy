    ...
    out.push(
      warning(
        ...
        "Podman did not respond. nomArmy starts a sandbox container per job, so worker jobs will not run " +
          "until Podman is available (on macOS, 'podman machine start'); its memory is still reserved in this plan.",
      ),
    );
  }
...
export function recommend(input = {}) {
  ...
  // separately computed number. Always guaranteed to fit -- `target` was
  // already stepped down (above) until at least 1 nom fits at it, so 1 nom
  // specifically can never need a further step-down.
  const nominal = {
