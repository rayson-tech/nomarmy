...
export function redactCommand(value, max) {
  ...
  // Authorization header values: Bearer/Basic/Token/ApiKey schemes, whether
  // typed raw or inside a quoted -H/--header argument.
  out = out.replace(/(\bauthorization\s*:\s*)(bearer|basic|token|apikey)(\s+)\S+/gi, "$1$2$3<redacted>");
  // -u/--user/--basic-auth with a user:pass-shaped value (colon, no
  // whitespace) -- e.g. `curl -u admin:hunter2`. Requiring the colon keeps
  // unrelated uses like `sort -u file.txt` (no colon in the value) untouched.
  out = out.replace(/(-u|--user|--basic-auth)([=\s]+)(\S+:\S+)/g, "$1$2<redacted>");
  // Database-CLI glued short-flag passwords, e.g. mysql's `-pSECRET` (no
  // separator between flag and value). Scoped to known DB client binaries
  // specifically: a bare `-p<word>` pattern would collide with unrelated
  // single-dash flags elsewhere (e.g. `go test -parallel 4`).
  if (/(^|[\s/])(mysql|mysqldump|psql|mongo|mongosh|redis-cli)\b/.test(out)) {
    out = out.replace(/(^|\s)-p(\S+)/g, "$1-p<redacted>");
  }
