...
/**
 * Production-code lines that carry on without an import: an added import
 * followed (within a few added lines) by any `except`, or an explicit
...
function stubbedImportLines(addedLines, deps) {
  ...
  addedLines.forEach((line, i) => {
    ...
    else if (EXCEPT_LINE_RE.test(line) && addedLines.slice(Math.max(0, i - 4), i).some((l) => IMPORT_LINE_RE.test(l))) hits.push(line);
    else {
