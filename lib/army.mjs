...
// Global configuration is army-only; the local file also admits the separately
// validated verification network policy. Both stay strict against unknown keys.
// The project file is .nomarmy.yml, whose other sections lib/schema.mjs owns.
const armyOnlyFileSchema = z.object({ army: armySchema.optional() }).strict();
// Verification owns validation and use of this operator-local section. Army
// updates must preserve it without permitting it in global configuration.
const localArmyFileSchema = armyOnlyFileSchema.extend({ verification_network: z.unknown().optional() });
...
export function readArmyFile(filePath, { armyOnly = false } = {}) {
  ...
  const doc = parsed ?? {};
  const result = armyOnly ? (path.basename(filePath) === LOCAL_CONFIG_FILENAME ? localArmyFileSchema : armyOnlyFileSchema).safeParse(doc) : armySchema.optional().safeParse(doc.army);
