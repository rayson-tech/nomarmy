 ...
 * @returns {{ok: boolean, message: string, fix?: string}}
 */
// Below this the Podman VM (macOS, Windows) can't hold a sandbox, a
// verification run and an image build at once. Seen live at 2 GiB: every
// sandbox command a worker ran came back "Aborted", reads included.
export const MIN_PODMAN_VM_MB = 4096;
const RECOMMENDED_PODMAN_VM_MB = 8192;

/**
 * Rootless Podman needs a range of subordinate IDs to run an image whose
 * files belong to other users (nomArmy's does). Seen live after a VM
 * restart: /etc/subuid and /etc/subgid emptied, one ID mapped, and every
 * sandbox failed with "error during chown ... permission denied".
 */
export function checkPodmanIdMappings(facts) {
  const ids = facts.podmanIdMappings;
  if (!ids || !ids.rootless) return { ok: true, message: "Podman ID mappings: not applicable (rootful, or Podman not answering)." };
  if (ids.uid >= 2 && ids.gid >= 2) return { ok: true, message: "Podman has its subordinate ID ranges." };
  return {
    ok: false,
    message: "Rootless Podman has no subordinate ID ranges (/etc/subuid, /etc/subgid are empty), so no sandbox can start: containers fail with \"error during chown ... permission denied\".",
    fix: "nomarmy sandbox --repair  (on Linux: add a range for your user to /etc/subuid and /etc/subgid, then run podman system migrate)",
  };
}

/** The Podman VM's memory: only where Podman runs in a VM (macOS, Windows). */
export function checkPodmanMachineMemory(facts) {
  const mb = facts.podmanMachineMemoryMb;
  if (!Number.isFinite(mb)) return { ok: true, message: "Podman VM memory: not applicable (Podman runs natively here, or no machine is running)." };
  const gib = (mb / 1024).toFixed(mb % 1024 ? 1 : 0);
  if (mb >= MIN_PODMAN_VM_MB) return { ok: true, message: `Podman VM has ${gib} GiB of memory.` };
  return {
    ok: false,
    message: `The Podman VM has only ${gib} GiB of memory. Sandboxes, verification and image builds share it, and at this size worker commands get cut off ("Aborted").`,
    fix: `nomarmy sandbox --memory ${RECOMMENDED_PODMAN_VM_MB / 1024}`,
  };
}
...
/** Rootless mode and how many UID/GID ranges Podman maps, or null. */
function probePodmanIdMappings(podmanPath, platform) {
  try {
    const result = spawnSync(podmanPath, ["info", "--format", "json"], { encoding: "utf8", timeout: 5000, shell: platform === "win32" && /\.(cmd|bat)$/i.test(podmanPath) });
    const host = JSON.parse(result.stdout || "{}").host ?? {};
    return { rootless: Boolean(host.security?.rootless), uid: host.idMappings?.uidmap?.length ?? 0, gid: host.idMappings?.gidmap?.length ?? 0 };
  } catch { return null; }
}

/** The running Podman machine's memory in MiB, or null (Linux, or no machine). */
function probePodmanMachineMemory(podmanPath, platform) {
  if (!["darwin", "win32"].includes(platform)) return null;
  try {
    const result = spawnSync(podmanPath, ["machine", "inspect"], { encoding: "utf8", timeout: 5000, shell: platform === "win32" && /\.(cmd|bat)$/i.test(podmanPath) });
    const machines = JSON.parse(result.stdout || "[]");
    const running = machines.find((m) => m.State === "running") ?? machines[0];
    const mb = Number(running?.Resources?.Memory);
