...
export function checkEndpoint(facts) {
  ...
  return {
    ...
    fix: "Set NOMARMY_EXECUTION to 'local', 'hosted', or 'bedrock' (see config/common.env).",
  };
}
...
export async function collectEndpointFacts(env = process.env, fetchImpl = fetch) {
  ...
  const execution = ["local", "hosted", "bedrock"].includes(configured) ? resolved.mode : configured;
