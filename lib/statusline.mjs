...
//   Opus 5.5 · nomarmy │ 🍪 idle · 2 in other repos
//
// This session's repo gets the detail; jobs in other repos are only a
// count (showing Senti's jobs in full in every window read as noise).
//
...
import { normalizeClaudeRateLimits, readUsageSnapshots, recordUsageSnapshot, usageStatus } from "./usage-limits.mjs";
...
export function statusLineText({ session = {}, stateRoot, now = Date.now(), maxLength = Number(process.env.NOMARMY_STATUSLINE_MAX) || 90 } = {}) {
  ...
  // Claude redraws this line often. Refresh the reading only when the
  // provider's actual windows changed, rather than rewriting on every draw.
  try {
    const snapshot = normalizeClaudeRateLimits(session.rate_limits);
    if (snapshot) {
      snapshot.observedAt = now;
      const previous = readUsageSnapshots(root)["claude-cli"];
      const windowValues = (s) => s?.windows.map(({ name, usedPercent, resetsAt }) => [name, usedPercent, resetsAt]);
      if (!previous || JSON.stringify(windowValues(previous)) !== JSON.stringify(windowValues(snapshot))) {
        recordUsageSnapshot(root, "claude-cli", snapshot);
      }
    }
  ...
  // A lease from before leases recorded their repo can't be attributed; it
  // counts as "elsewhere" rather than claimed by every window.
  // A lease from a server that predates `repo`: its worktree's .git file
  // ("gitdir: <repo>/.git/worktrees/<name>") still names the repo.
  for (const l of leases) {
    if (l.repo) continue;
    try {
      const m = /^gitdir:\s*(.+?)[\\/]\.git[\\/]worktrees[\\/]/.exec(fs.readFileSync(path.join(root, "jobs", l.jobId, "worktree", ".git"), "utf8"));
      if (m) l.repo = m[1];
    } catch { /* no worktree yet */ }
  }
  const inRepo = (l) => Boolean(l.repo) && (cwd === l.repo || cwd.startsWith(`${l.repo}${path.sep}`));
  const elsewhere = leases.filter((l) => !inRepo(l)).length;
  leases = leases.filter(inRepo);
  ...
  // The most serious recent health warning (lib/health.mjs), short.
  let healthPart = "";
  try {
  ...
  try {
    const warnings = Object.entries(readUsageSnapshots(root)).map(([provider, snapshot]) => {
      ...
      const window = live[0];
      const amount = window ? `${window.usedPercent}% ${window.name === "week" ? "wk" : window.name}` : "limit";
      return { level: status.level, text: ` │ ${status.level === "over" ? "⛔" : "⚠"} ${provider} ${amount}` };
