...
export function publicAddress(address) {
  ...
  if (parseInt(parts[5], 16) === 0x5efe || parseInt(parts[4], 16) === 0x5efe) return false;
...
export function createEgressProxy({ allow, token, resolve = host => lookup(host, { all: true }), log = () => {}, connect = opts => net.connect(opts), request = opts => http.request(opts) }) {
  if (typeof token !== 'string' || !token) throw new Error('proxy token required');
  const digest = value => createHash('sha256').update(value).digest();
  const expected = digest('Basic ' + Buffer.from('nomarmy:' + token).toString('base64'));
  const authenticated = req => timingSafeEqual(expected, digest(String(req.headers['proxy-authorization'] ?? '')));
  ...
  const server = http.createServer(async (req, res) => {
    if (!authenticated(req)) { res.writeHead(407, { 'Proxy-Authenticate': 'Basic realm="verification"' }); res.end(); return; }
    ...
    upstream.on('socket', socket => socket.once('connect', () => log(`${destination.authority} connected`)));
    upstream.on('response', reply => { res.writeHead(reply.statusCode, reply.headers); reply.pipe(res); });
    upstream.on('error', () => { log(`${destination.authority} denied`); if (!res.headersSent) res.writeHead(502); res.end(); });
  ...
  server.on('connect', async (req, client, head) => {
    client.on('error', () => {});
    if (!authenticated(req)) { client.end('HTTP/1.1 407 Proxy Authentication Required\r\nProxy-Authenticate: Basic realm="verification"\r\nConnection: close\r\n\r\n'); return; }
    let destination;
    ...
    upstream.setTimeout(30_000, () => upstream.destroy());
    upstream.on('error', () => { log(`${destination.authority} denied`); client.destroy(); });
    client.on('close', () => upstream.destroy());
    upstream.on('close', () => client.destroy());
    upstream.on('connect', () => {
      log(`${destination.authority} connected`);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  createEgressProxy({ token: process.env.NOMARMY_EGRESS_TOKEN, allow: JSON.parse(process.env.NOMARMY_EGRESS_ALLOW), log: line => process.stdout.write(`${line}\n`) }).listen(3128, '0.0.0.0');
}
