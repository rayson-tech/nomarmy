...
function pythonProjectLayer(cwd, manager, files, secrets = []) {
  ...
  let install;
  if (manager === "uv") {
    install = `pip3 install --no-cache-dir --break-system-packages${credentialed ? " --only-binary=:all:" : ""} uv && uv sync --frozen --no-install-project --all-groups${credentialed ? " --no-build" : ""}`;
...
export function dependencyDockerfile({ requirements = [], nodeFiles = [], nodePackages = null, nodeInstalls = null, secrets = [] }) {
  ...
  const pip = requirements.length ? registryInstallRun(`pip3 install --no-cache-dir --break-system-packages ${secrets.some(s => s.ecosystem === "pip") ? "--only-binary=:all: " : ""}${requirements.map((_, i) => `-r ${reqRoot}/req-${i}.txt`).join(" ")}${secrets.some(s => s.ecosystem === "pip") ? " || touch /deps/requirements/.nomarmy-pip-install-failed" : ""}`, "pip", secrets) : "";
  ...
  const installs = managers.map(({ dir, manager, yarnFallback }) => {
    ...
    if (secrets.some(secret => secret.ecosystem === "npm")) {
      ...
      return [
        ...(yarnFallback ? [`RUN corepack prepare yarn@${yarnFallback} --activate`] : []),
...
export function composeSandboxImage(cwd, config = null, selection = sandboxHarnesses(cwd, config), options = null) {
  ...
  if (options === null) return composeWithRegistrySecrets(cwd, config, selection, readRegistrySecrets(cwd));
