...
export function composeSandboxImage(cwd, config = null, selection = sandboxHarnesses(cwd, config), options = null) {
  ...
  return trustedComposition(cwd, config, selection, readRegistrySecrets(options.trustedDir ?? cwd), options.trustedDir).composed;
}
...
export function ensureComposedImageBuilt(cwd, config, { run = defaultRun, trustedDir, onNote = () => {} } = {}) {
  ...
  // read only to report that credentials weren't used (trustedComposition
  // fails closed with no trustedDir).
  const declared = readRegistrySecrets(trustedDir ?? cwd);
