// Lazy, content-addressed sandbox images composed from matched harnesses.
...
export function composeSandboxImage(cwd, config = null, selection = sandboxHarnesses(cwd, config)) {
  ...
  for (const { source, destination } of files) {
    hash.update(source).update("\0").update(destination).update("\0")
      .update(fs.readFileSync(path.join(cwd, source))).update("\0");
  }
...
export function ensureComposedImageBuilt(cwd, config, { run = defaultRun } = {}) {
  ...
  try {
    for (const { source, destination } of composed.files) {
      const target = path.join(context, destination);
      fs.mkdirSync(path.dirname(target), { recursive: true });
      fs.copyFileSync(path.join(cwd, source), target);
    }
