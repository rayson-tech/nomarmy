...
export function composeSandboxImage(cwd, config = null, selection = sandboxHarnesses(cwd, config)) {
  return composeWithRegistrySecrets(cwd, config, selection, readRegistrySecrets(cwd));
}
...
export function ensureComposedImageBuilt(cwd, config, { run = defaultRun } = {}) {
  ...
  const composed = composeWithRegistrySecrets(cwd, config, sandboxHarnesses(cwd, config), secrets);
  ...
  try {
    ...
    throw new Error(`failed to build sandbox harness image (${composed.image}): ${secrets.length ? "credentialed build failed (output withheld)" : error.message}`);
  } finally {
    fs.rmSync(context, { recursive: true, force: true });
  }
