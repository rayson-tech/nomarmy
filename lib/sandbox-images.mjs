...
function pythonProjectLayer(cwd, manager, files) {
...
RUN cd /deps/python && (python3 -m venv /deps/python/.venv && ${manager === "poetry" ? "export VIRTUAL_ENV=/deps/python/.venv && " : ""}${install} || touch .nomarmy-${manager}-install-failed)
...
export function dependencyDockerfile({ requirements = [], nodeFiles = [], nodePackages = null, nodeInstalls = null }) {
  ...
  const installs = managers.map(({ dir, manager, yarnFallback }) => {
    ...
    if (command) return `RUN cd ${shellQuote(at)} && (${yarnFallback ? `corepack prepare yarn@${yarnFallback} --activate && ` : ""}${command} || touch .nomarmy-${manager}-install-failed)`;
...
export function sandboxHarnesses(cwd, config = null, harnesses = loadHarnesses().harnesses) {
  ...
  const matched = matchHarnesses(cwd, harnesses);
...
function goDependencyLayer(cwd, files) {
...
function rustDependencyLayer(cwd, files) {
  const manifests = [];
  const walk = (dir, depth) => {
...
USER node
RUN cd /deps/rust && (cargo fetch${locked ? " --locked" : ""} || touch .nomarmy-cargo-fetch-failed)
 ...
 */
export function composeSandboxImage(cwd, config = null, selection = sandboxHarnesses(cwd, config)) {
  ...
  const dependencyLayer = (options) => dependencyDockerfile(options).slice(base.length, -footer.length).trim();
  const layers = [], files = [];
  ...
  for (const name of order) {
    ...
    if (image.builtin === "go" || image.builtin === "rust") {
      ...
      layer += "\n" + (image.builtin === "go" ? goDependencyLayer(cwd, files) : rustDependencyLayer(cwd, files));
    } else if (image.builtin === "python") {
      const requirements = pythonRequirementsFor(cwd, config);
      const manager = requirements.length ? null : pythonProjectManager(cwd);
      if (manager) {
        layer = pythonProjectLayer(cwd, manager, files);
        pythonVenv = true;
      } else {
  ...
  const dockerfile = base + layers.join("\n") + "\n" + runtime + footer;
...
export function ensureComposedImageBuilt(cwd, config, { run = defaultRun } = {}) {
  const composed = composeSandboxImage(cwd, config);
  ...
  try {
    for (const { source, destination, content } of composed.files) {
      ...
      else fs.copyFileSync(path.join(cwd, source), target);
    }
    fs.writeFileSync(path.join(context, "Dockerfile"), composed.dockerfile);
    run("podman", ["build", "-t", composed.image, "-f", path.join(context, "Dockerfile"), context]);
  } catch (error) {
    throw new Error(`failed to build sandbox harness image (${composed.image}): ${error.message}`);
  } finally {
    fs.rmSync(context, { recursive: true, force: true });
  }
...
export function resolveSandboxImage({ cwd, explicitImage, defaultImage, config = null, run = defaultRun }) {
  ...
  return ensureComposedImageBuilt(cwd, config, { run }) ?? defaultImage;
}
