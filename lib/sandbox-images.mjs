...
import { requirementsInputs, validatePythonProject } from "./registry-python.mjs";
import { readRegistrySecrets, hasRegistryDeclarations, assertNoRegistryCopies, registryInstallRun } from "./registry-secrets.mjs";
...
export function pythonRequirementsFor(cwd, config) {
  ...
  if (Array.isArray(declared) && declared.length > 0) {
    declared.forEach(source => assertRepositorySource(cwd, source, true));
    return declared;
  }
...
function pythonProjectLayer(cwd, manager, files, secrets = []) {
  const credentialed = secrets.some(secret => secret.ecosystem === "pip");
  if (credentialed && manager === "poetry") {
    throw new Error("registries: poetry installs can't be made build-free; use uv, or wheels via pip");
  }
  if (credentialed) validatePythonProject(cwd, manager);
  ...
  let install;
  if (manager === "uv") {
    install = credentialed ? "uv sync --frozen --no-install-project --all-groups --no-build" : "pip3 install --no-cache-dir --break-system-packages uv && uv sync --frozen --no-install-project --all-groups";
    ...
    const deps = readProject + 'import subprocess; p=d.get("project", {}); extras=p.get("optional-dependencies", {}); deps=p.get("dependencies", []) + [dep for name in ("test", "tests", "dev") for dep in extras.get(name, [])]; subprocess.check_call(["/deps/python/.venv/bin/python", "-m", "pip", "install", "--no-cache-dir", ' + (credentialed ? '"--only-binary=:all:", ' : '') + '"--", *deps]) if deps else None';
    install = `python3 -c ${shellQuote(deps)}`;
  }
  return `COPY py/ /deps/python/
ENV UV_PROJECT_ENVIRONMENT=/deps/python/.venv
ENV POETRY_VIRTUALENVS_CREATE=false
${credentialed ? "RUN python3 -m venv /deps/python/.venv\n" + (manager === "uv" ? "RUN pip3 install --no-cache-dir --break-system-packages --only-binary=:all: --index-url https://pypi.org/simple uv==0.8.22\n" : "") : ""}${registryInstallRun(`cd /deps/python && (${credentialed ? "" : "python3 -m venv /deps/python/.venv && "}${manager === "poetry" ? "export VIRTUAL_ENV=/deps/python/.venv && " : ""}${install} || touch .nomarmy-${manager}-install-failed)`, "pip", secrets)}
...
export function dependencyDockerfile({ requirements = [], nodeFiles = [], nodePackages = null, nodeInstalls = null, secrets = [], requirementCount = requirements.length, requirementIndexes = requirements.map((_, i) => i) }) {
  const reqRoot = secrets.some((s) => s.ecosystem === "pip") ? "/deps/requirements" : "/tmp/reqs";
  const reqCopies = Array.from({ length: requirementCount }, (_, i) => `COPY py/req-${i}.txt ${reqRoot}/req-${i}.txt`).join("\n");
  const pip = requirements.length ? registryInstallRun(`pip3 install --no-cache-dir --break-system-packages ${secrets.some(s => s.ecosystem === "pip") ? "--only-binary=:all: " : ""}${requirementIndexes.map(i => `-r ${reqRoot}/req-${i}.txt`).join(" ")}${secrets.some(s => s.ecosystem === "pip") ? " || touch /deps/requirements/.nomarmy-pip-install-failed" : ""}`, "pip", secrets) : "";
  ...
  const installs = managers.map(({ dir, manager, yarnFallback, packageManager }) => {
    ...
    if (secrets.some(secret => secret.ecosystem === "npm")) {
      const marker = manager === "npm" ? ".nomarmy-npm-ci-failed" : `.nomarmy-${manager}-install-failed`;
      const install = command ?? "npm ci --no-audit --no-fund";
      // Bun creates an npm-compatible node_modules tree. npm rebuild actually
      // runs its dependency lifecycle scripts; bun pm untrusted only lists them.
      const rebuild = manager === "pnpm" ? "pnpm rebuild" : "npm rebuild";
      return [
        ...((manager === "pnpm" || manager.startsWith("yarn")) ? [`RUN cd ${shellQuote(at)} && ${packageManager ? "corepack install" : `${yarnFallback ? `corepack prepare yarn@${yarnFallback} --activate && ` : ""}${manager} --version`}`] : []),
        registryInstallRun(`cd ${shellQuote(at)} && (${install} --ignore-scripts || touch ${marker})`, "npm", secrets),
        // The mount must be gone before any dependency code executes. No
        // scripts can copy its contents during the preceding install.
        `RUN cd ${shellQuote(at)} && ((test ! -e /home/node/.npmrc && test ! -L /home/node/.npmrc) || { touch ${marker}; exit 1; }) && (${rebuild} || touch ${marker})`,
      ].join("\n");
    }
...
function assertRepositorySource(cwd, source, allowMissing = false) {
  const fail = () => { throw new Error("sandbox dependency source must be a repository-contained regular file (no absolute or escaping paths)"); };
  if (typeof source !== "string" || !source || path.isAbsolute(source) ||
      source.split(/[\\/]/).includes("..")) fail();
  if (repositoryFile(cwd, source)) return;
  if (allowMissing) {
    // Language detection also accepts not-yet-created requirements. Check the
    // nearest existing ancestor, including symlinks, before allowing absence.
    const root = fs.realpathSync(cwd);
    let target = path.resolve(cwd, source);
    while (target.startsWith(path.resolve(cwd) + path.sep)) {
      try {
        fs.lstatSync(target);
      } catch (error) {
      ...
      let real;
      try { real = fs.realpathSync(target); } catch { fail(); }
      if (target !== path.resolve(cwd, source) &&
          real.startsWith(root + path.sep) && fs.statSync(target).isDirectory()) return;
      fail();
    }
    if (target === path.resolve(cwd) && target !== path.resolve(cwd, source)) return;
  }
  fail();
}
...
export function composeSandboxImage(cwd, config = null, selection = sandboxHarnesses(cwd, config), options = null) {
  ...
  if (options === null) return composeWithRegistrySecrets(cwd, config, selection, []);
  // A build-shaped call reads declarations the way ensureComposedImageBuilt does.
  return trustedComposition(cwd, config, selection, options.trustedDir ? readRegistrySecrets(options.trustedDir) : [], options.trustedDir).composed;
}
...
function trustedComposition(cwd, config, selection, secrets, trustedDir) {
  ...
  if (!trustedDir) {
    const note = "private-registry credentials were not used: this job changed dependency inputs (trusted checkout unavailable)";
    return { composed: plain && hasRegistryDeclarations(cwd) ? { ...plain, note } : plain, secrets: [] };
  }
...
function composeWithRegistrySecrets(cwd, config, selection, secrets) {
  ...
  for (const name of order) {
    ...
    if (image.builtin === "go" || image.builtin === "rust") {
      ...
      if (manager) {
        ...
        const inputs = requirementsInputs(cwd, requirements, secrets.some(s => s.ecosystem === "pip"));
        files.push(...inputs.files);
        layer = dependencyLayer({ requirements, requirementCount: inputs.files.length, requirementIndexes: inputs.indexes });
      }
      ...
      const installs = node.installs?.map(install => ({ ...install,
        packageManager: Boolean(JSON.parse(fs.readFileSync(path.join(cwd, install.dir, "package.json"), "utf8")).packageManager),
      }));
      layer = dependencyLayer({ nodeFiles: node.files, nodePackages: node.packages, nodeInstalls: installs });
  ...
  for (const { source } of files) if (source !== undefined) assertRepositorySource(cwd, source);
...
export function ensureComposedImageBuilt(cwd, config, { run = defaultRun, trustedDir, onNote = () => {} } = {}) {
  // With a trusted checkout, declarations come only from it: a job's worktree
  // is worker-writable, and a .nomarmy.local.yml there must never choose
  // which host file gets mounted into a build. Without one, a declaration is
  // detected by its YAML key only; credential paths are never inspected.
  const declared = trustedDir ? readRegistrySecrets(trustedDir) : [];
