...
export function createVerificationFlow(deps) {
  ...
  async function runRegressionCheck({ cwd, jobId, productionFiles, nameStatus, profile, baseSha, branch, mode, verificationResult }) {
    ...
    if (rerun.network && verificationResult?.network) {
      verificationResult.network.reached = [...new Set([...(verificationResult.network.reached ?? []), ...(rerun.network.reached ?? [])])];
      if (rerun.log && verificationResult.log) {
        try { fs.appendFileSync(verificationResult.log, "\n\n[regression check]\n" + fs.readFileSync(rerun.log, "utf8")); }
        catch { rerun = { ...rerun, status: "not_run", basis: "egress-log-failed", reason: "could not merge regression network log" }; }
      }
    }
    const rerunEvidence = rerun.network ? { network: rerun.network, log: rerun.log, issues: rerun.issues } : {};
    ...
    if (restoreErrors.length > 0 || mismatches.length > 0) {
      return { ...rerunEvidence, status: "restore_failed", rawRerunStatus: rerun.status, basis: "restore-error",
    ...
    if (rerun.status === "fail") return { ...rerunEvidence, status: "pass", rawRerunStatus: "fail", basis: rerun.basis, reason: "reverting the production change made the same verification profile fail, as expected -- a test catches this regression", detail: rerun.detail };
    if (rerun.status === "pass") return { ...rerunEvidence, status: "fail", rawRerunStatus: "pass", basis: rerun.basis, reason: "verification still passed with the production change reverted -- no test demonstrably catches this regression", detail: rerun.detail };
    return { ...rerunEvidence, status: "not_run", rawRerunStatus: "not_run", basis: rerun.basis, reason: `regression rerun was inconclusive: ${rerun.reason}`, detail: rerun.detail };
  }
  ...
  function normalizeVerification(value, profile = null) {
    ...
    return {
      ...
      ...(value?.network ? { network: value.network, issues: value.issues ?? [] } : {})
    };
  }
  async function runIndependentVerification(context) {
    ...
    try {
      const artifactJobDir = context.jobDir ?? (context.logFile ? path.dirname(context.logFile) :
        context.jobId && ensureJobsRoot ? path.join(ensureJobsRoot(), context.jobId) : null);
      const value = await verificationRunner({ ...context, jobDir: artifactJobDir });
      const normalized = normalizeVerification(value, context.profile);
      // Networked runs must retain the proxy verdicts for every job mode.
      if (!context.logFile && value?.network && context.jobId && ensureJobsRoot) {
        context = { ...context, logFile: path.join(ensureJobsRoot(), context.jobId, "verification.log") };
      }
      // Persist only the runner-sanitized output (credentialed runs withhold it).
      if (context.logFile && typeof value?.output === "string") {
        try {
          if (value?.network) fs.mkdirSync(path.dirname(context.logFile), { recursive: true });
          fs.writeFileSync(context.logFile, value.output);
          normalized.log = context.logFile;
        } catch {
          if (value?.network) return { ...normalized, status: "not_run", basis: "egress-log-failed", reason: "could not retain verification network log", detail: null };
          // Non-networked logs remain best-effort for compatibility.
        }
      }
      ...
      if (jobDir && context.cwd) {
        try {
          Object.assign(normalized, value?.artifacts ? {
            artifacts: value.artifacts, artifactsCapped: value.artifactsCapped,
            ...(value.artifactsNote ? { artifactsNote: value.artifactsNote } : {})
          } : value?.network?.credentials?.length ? { artifacts: [], artifactsCapped: false } : collectVerificationArtifacts(context.cwd, jobDir));
