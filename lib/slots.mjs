...
export function writeLease(leasesDir, jobId, { lane, agent = null }) {
  fs.mkdirSync(leasesDir, { recursive: true });
  fs.writeFileSync(path.join(leasesDir, `${safeKey(jobId)}.json`), JSON.stringify({ jobId, lane, agent, pid: process.pid, startedAt: new Date().toISOString() }));
}
...
export function liveLeases(leasesDir, { lane = null, agent = null, alive = pidAlive } = {}) {
  let files;
