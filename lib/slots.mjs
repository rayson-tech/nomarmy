...
export function writeLease(leasesDir, jobId, { lane, agent = null, runId = null, role = null, model = null, repo = null }) {
  fs.mkdirSync(leasesDir, { recursive: true });
  fs.writeFileSync(path.join(leasesDir, `${safeKey(jobId)}.json`), JSON.stringify({ jobId, lane, agent, runId, role, model, repo, pid: process.pid, startedAt: new Date().toISOString() }));
}
