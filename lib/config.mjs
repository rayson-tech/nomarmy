...
import { validateEnabledHarnesses } from "./harnesses.mjs";
import { readRegistrySecrets } from "./registry-secrets.mjs";
...
export function validateConfig(input) {
  ...
  if (Object.hasOwn(candidate, "verification_network")) {
    return { valid: false, config: null, errors: ["verification_network is operator-local only: put it in gitignored .nomarmy.local.yml, never .nomarmy.yml"], elevated: emptyElevated() };
  }
  if (Object.hasOwn(candidate, "registries")) {
    return { valid: false, config: null, errors: ["registries: allowed only in .nomarmy.local.yml, never in committed repository configuration"], elevated: emptyElevated() };
  }
  ...
  try { validateEnabledHarnesses(result.data.harnesses); }
  catch (error) {
    return { valid: false, config: null, errors: [error.message], elevated: emptyElevated() };
  }
  return {
    valid: true,
    config: result.data,
...
export function loadConfig(repoDir = process.cwd()) {
  try { readRegistrySecrets(repoDir); }
  catch (error) { throw new ConfigError(error.message, { errors: [error.message] }); }
