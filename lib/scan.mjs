...
const GO_MOD_RE = /^go\.mod$/;
const CARGO_TOML_RE = /^Cargo\.toml$/;
...
function classifyFile(relPath) {
  ...
  if (GO_MOD_RE.test(base)) return "go-mod";
  if (CARGO_TOML_RE.test(base)) return "cargo-toml";
...
function handleGoMod(text, source, raw) {
  const body = String(text).replace(/\r\n?/g, "\n");
  const match = body.match(/^\s*module\s+(\S+)/m);
  raw.tooling.push({ name: "go", category: "package-manager", detail: match ? `module ${match[1]}` : source, source });
  raw.commands.push({ name: "go build", kind: "build", command: "go build ./...", source });
  raw.commands.push({ name: "go test", kind: "test", command: "go test ./...", source });
}

function handleCargoToml(text, source, raw) {
  const body = String(text).replace(/\r\n?/g, "\n");
  const section = body.split(/^\s*\[/m).find((s) => /^package\]/.test(s)) ?? "";
  const match = section.match(/^\s*name\s*=\s*["']([^"']+)["']/m);
  raw.tooling.push({ name: "cargo", category: "package-manager", detail: match ? `package ${match[1]}` : source, source });
  raw.commands.push({ name: "cargo build", kind: "build", command: "cargo build", source });
  raw.commands.push({ name: "cargo test", kind: "test", command: "cargo test", source });
}
 ...
 * normal; a repo containing none of the recognized files still returns a valid
...
function dispatch(kind, text, source, raw) {
  switch (kind) {
    ...
    case "requirements":
      ...
      break;
    case "go-mod":
      handleGoMod(text, source, raw);
      break;
    case "cargo-toml":
      handleCargoToml(text, source, raw);
      break;
    case "makefile":
