...
import { dispatchConfigSchema, formatDispatchIssues, findReservedPoolName, openclawProviderId } from "./dispatch-schema.mjs";
...
/** Where providers.yml lives in a config directory (normally
 * ~/.config/nomarmy -- see lib/army.mjs's globalConfigDir), whether or not
 * it exists yet. */
export function dispatchConfigPath(configDir) {
  return path.join(configDir, DISPATCH_CONFIG_FILENAME);
}

/**
 * Find, parse and validate providers.yml in `configDir`.
 * @param {string} configDir
 * @returns {{ found: boolean, path: string|null, config: { pools: object }|null }}
 */
export function loadDispatchConfig(configDir) {
  const configPath = dispatchConfigPath(configDir);
  if (!isReadableFile(configPath)) {
    return { found: false, path: null, config: null };
  }

  const unsafe = privateConfigProblem(configPath);
  if (unsafe) throw new DispatchConfigError(unsafe, { path: configPath, errors: [`config: ${unsafe}`] });
...
export function resolveEntryContext(entry, { catalog = null } = {}) {
  ...
  if (entry.context_window) return { raw: entry.context_window, source: `config/providers.yml override (${entry.id})` };
  ...
  return {
    ...
    source: `unknown model "${key}" -- not in openclaw's cached catalog and no context_window override set (run \`openclaw models list --refresh\`, or set context_window in config/providers.yml); using a conservative ${UNKNOWN_MODEL_CONTEXT_FALLBACK}-token fallback`,
  };
}
