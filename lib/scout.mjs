...
- When the question asks about a function, a decision or a behaviour, read the source files that implement it. Documentation (README, CLAUDE.md, AGENTS.md, policies) describes intent; it is not evidence of what the code does.
- Every FINDING must cite where you saw it: the file path relative to the repository root, a colon, then the line number or line range you read. For example [lib/config.mjs:41-58] or [bin/nomarmy.mjs:120]. Use real file names and real line numbers; never write the words "path", "start" or "end". The coordinator resolves each citation against the snapshot and attaches the cited lines; a finding whose citation does not resolve is discarded as hearsay.
...
export async function verifyCitations(findings, { readFile, limits = DEFAULT_SCOUT_LIMITS }) {
  ...
  for (const f of findings ?? []) {
    ...
    for (const c of f.citations ?? []) {
      ...
      // finding's distinctive terms at all? Heuristic, and labelled as such.
