...
function globBody(glob) {
  ...
  for (let i = 0; i < glob.length; i++) {
    const ch = glob[i];
    if (ch === "*") { if (glob[i + 1] === "*") { re += glob[i + 2] === "/" ? "(?:.*/)?" : ".*"; i++; if (glob[i + 1] === "/") { i++; } } else re += "[^/]*"; }
    else if (ch === "?") re += "[^/]";
...
export function formatCitations(result) {
  ...
  if (result.op === "files") {
    ...
    if (!result.items?.length) out.push(result.exists ? `(no definitions recognized in ${result.path}; ${result.lines ?? "?"} lines)` : `(no such file: ${result.path})`);
