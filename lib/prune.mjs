 ...
 * @returns {{ pruned: number, scratchCleared: number, freedBytes: number }}
 */
export function pruneJobRuntime({ stateRoot, olderThanMs, now = Date.now() }) {
  ...
  let pruned = 0, scratchCleared = 0, freedBytes = 0, names = [];
  try { names = fs.readdirSync(jobsRoot); } catch { return { pruned, scratchCleared, freedBytes }; }
  for (const name of names) {
    ...
    // OpenClaw's scratch space (plugin build copies, 1.2 GB for a Codex
    // job) is of no use once the job is done, whatever its age; the
    // transcript a recent job might still need isn't in it.
    const scratch = path.join(runtime, "state", "tmp");
    if (fs.existsSync(scratch)) {
      freedBytes += sizeOf(scratch);
      fs.rmSync(scratch, { recursive: true, force: true });
      scratchCleared++;
    }
  }
  return { pruned, scratchCleared, freedBytes };
}
