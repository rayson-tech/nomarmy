// nomArmy api provider-entry schema (v1.3).
//
// This module owns the per-provider rules for an api agent: which provider
// types exist, which need base_url, how the generic `openclaw` type names
// its provider id. lib/agents.mjs validates every `kind: api` agent in
// agents.yml as one of these entries (a one-entry pool, the shape the
// dispatch path was written for). No filesystem access, no YAML.
//
...
// `agents add` wizard can validate as-you-type and re-prompt immediately,
 ...
 * `openclaw` is the generic form of that for any OTHER provider OpenClaw
 * knows, or gets from a plugin (DeepSeek, Mistral, Groq, ...), named by
 * `openclaw_provider` so nomArmy never has to enumerate them;
...
/** OpenClaw provider ids: lowercase letters, digits, dot, underscore, hyphen. */
export const OPENCLAW_PROVIDER_ID_RE = /^[a-z0-9][a-z0-9._-]{0,63}$/;

/**
 * The provider id OpenClaw itself knows this entry by -- what goes in front
 * of the model (`<id>/<model>`) and what its credential is stored under.
 * Every type is its own id except the generic `openclaw` one.
 */
export function openclawProviderId(entry) {
  return entry.provider === "openclaw" ? entry.openclaw_provider : entry.provider;
}

/** Types that register straight onto an existing OpenClaw provider id, with no custom endpoint to define first. */
export function isNativeProviderType(type) {
  return type === "openclaw" || NATIVE_PROVIDER_TYPES.includes(type);
}
...
// Any other OpenClaw provider. `plugin` is the install spec when the
// provider isn't stock (e.g. "clawhub:@openclaw/deepseek-provider");
...
/**
 * Providers whose worker runs its own tools on this machine, outside
 * nomArmy's sandbox. OpenClaw runs the Claude CLI (claude-cli) as the real
 * `claude` binary on the host and approves its built-in Bash, Edit and
 * Write itself: probed live, its shell reported Darwin, the real worktree
 * path and network access (HTTP 200), where every other route (local, xai,
 * openai via Codex, meta) reported Linux, /workspace and no network. A real
 * Senti job on it ran `npm install` on the host and replaced the worktree's
 * dependency link with macOS binaries. An api `anthropic` key runs through
 * OpenClaw's own loop and is sandboxed like the rest.
 */
export const HOST_TOOL_PROVIDERS = Object.freeze(["claude-cli"]);
export function agentRunsToolsOnHost(agent) {
  return Boolean(agent && agent.kind === "subscription" && HOST_TOOL_PROVIDERS.includes(agent.provider));
}
