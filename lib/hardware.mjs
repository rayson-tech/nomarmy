...
//   * podman info --format {{.Host.MemTotal}}
...
/** `podman info` can hang while a macOS Podman machine is still starting up. */
const PODMAN_TIMEOUT_MS = 6000;
...
/**
 * Parse `vm_stat` into memory available without swapping: free + inactive +
 * speculative + purgeable pages, scaled by the page size vm_stat itself
 * reports. `os.freemem()` on Darwin counts only free + speculative, which
 * ignores tens of GiB of reclaimable file-backed cache the kernel evicts
 * instantly under pressure -- the same machine can report single-digit GiB
 * "free" while `memory_pressure` says it is nowhere near constrained. This
 * mirrors macOS's own `MemAvailable`-equivalent notion, the same way
 * `parseProcMeminfoBytes` does for Linux.
 *
 * Some inactive pages are dirty and need a writeback/compression pass before
 * they are truly free, so this can slightly overstate available memory. That
 * is still far closer to reality than freemem's undercount.
 *
 * Returns null if the page size or any of the four fields cannot be found,
 * so the caller can fall back rather than silently mis-measuring.
 *
 * @param {string} stdout
 * @returns {number|null}
 */
export function parseVmStatBytes(stdout) {
  ...
  if (![free, inactive, speculative, purgeable].every((n) => Number.isFinite(n))) return null;
...
/**
 * Parse `podman info --format {{.Host.MemTotal}}` output (bytes). On macOS
 * this is the Podman machine VM's total memory (its configured ceiling, seen
 * from inside); on native Linux it is just the real host total. Same bare-
 * integer output shape as Docker's equivalent `{{.MemTotal}}` field, so the
 * parsing logic is unchanged from before, only the field path differs.
 *
 * @param {string} stdout
 * @returns {number|null}
 */
export function parsePodmanMemTotal(stdout) {
 ...
 *
 * @param {{ skipPodman?: boolean, skipGpu?: boolean }} [options]
...
export function detectHardware(options = {}) {
  ...
  const availableMemoryBytes = probe(
    ...
    (entry) => {
      ...
      if (platform === "darwin") {
        const res = runCommand("vm_stat", []);
        if (res.ok) {
          const parsed = parseVmStatBytes(res.stdout);
          if (parsed != null) {
            entry.source = "vm_stat (free+inactive+speculative+purgeable pages)";
            entry.detail =
              "Reclaimable-inclusive, closer to macOS's own notion of available memory than node:os freemem, " +
              "which counts only free+speculative and ignores tens of GiB of evictable file-backed cache. " +
              "Some inactive pages are dirty and need a writeback/compression pass first, so this can slightly " +
              "overstate what is instantly free.";
            return parsed;
          }
        }
      }
      ...
      if (platform === "darwin") {
        entry.detail =
          "macOS freemem excludes reclaimable cache and compressed pages, so it understates what is really " +
          "available; vm_stat was unavailable or unparseable, so this is a conservative fallback";
      }
  ...
  const podman = probe(probes, "podman.info", "Podman availability and reported memory", (entry) => {
    if (options.skipPodman) {
      entry.detail = "skipped by caller";
      return null;
    }
    entry.source = "podman info --format {{.Host.MemTotal}}";
    const res = runCommand("podman", ["info", "--format", "{{.Host.MemTotal}}"], PODMAN_TIMEOUT_MS);
    if (!res.ok) {
      entry.detail = `podman unavailable or not responding (${res.reason})`;
      return { available: false, totalMemoryBytes: null };
    }
    return { available: true, totalMemoryBytes: parsePodmanMemTotal(res.stdout) };
  });
  ...
  return {
    ...
    memory: {
      ...
      // model weights, KV cache, the OS, the coordinator and every Podman
    ...
    podman: podman || { available: null, totalMemoryBytes: null },
    probes,
  };
}
