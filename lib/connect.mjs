 ...
 */
export function connectClaude({ nomarmyRoot, installDir = defaultInstallDir(), run = defaultRun, extraEnv = {}, configDir = globalConfigDir(), commandsDir = defaultCommandDirs().claude }) {
  ...
  return { installDir, serverPath, preservedEnv: finalEnv, commands };
}
...
export function connectCodex({ nomarmyRoot, installDir = defaultInstallDir(), run = defaultRun, skillsDir = defaultCommandDirs().codex }) {
  ...
  run("codex", ["mcp", "list"]);
  return { installDir, serverPath, commands };
}
...
export function connectCursor({ nomarmyRoot, installDir = defaultInstallDir(), configPath = defaultCursorConfigPath(), run = defaultRun, commandsDir = defaultCommandDirs().cursor }) {
  ...
  return { installDir, serverPath, configPath, preservedEnv, commands };
}
