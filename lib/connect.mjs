...
export function installMcpCopy({ nomarmyRoot, installDir, run = defaultRun }) {
  ...
  // checkout's config/ directory. common.env/profiles/*.env are NOT copied
  // here on purpose: those reach the server only through connect-time -e
  // env injection (see deriveWorkerModelEnv below), never a live file read.
...
export function connectClaude({ nomarmyRoot, installDir = defaultInstallDir(), run = defaultRun }) {
  ...
  const finalEnv = { ...preservedEnv, ...deriveWorkerModelEnv(nomarmyRoot) };
