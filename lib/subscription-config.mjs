...
/** Where subscriptions.yml lives in a config directory (normally
 * ~/.config/nomarmy), whether or not it exists yet. */
export function subscriptionConfigPath(configDir) {
  return path.join(configDir, SUBSCRIPTION_CONFIG_FILENAME);
}

/**
 * Find, parse and validate subscriptions.yml in `configDir`.
 * @param {string} configDir
 * @returns {{ found: boolean, path: string|null, config: { workers: object }|null }}
 */
export function loadSubscriptionConfig(configDir) {
  const configPath = subscriptionConfigPath(configDir);
  if (!isReadableFile(configPath)) {
    return { found: false, path: null, config: null };
  }

  const unsafe = privateConfigProblem(configPath);
  if (unsafe) throw new SubscriptionConfigError(unsafe, { path: configPath, errors: [`config: ${unsafe}`] });
