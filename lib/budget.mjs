 ...
 */
export function deriveBudgets({ contextPerNom = null, source = "default", env = process.env } = {}) {
  ...
  const briefTokens = clamp(Math.floor(ctx * R.briefFraction), R.briefTokensMin, R.briefTokensMax);
  let maxTaskChars = briefTokens * cpt;
  let maxAcceptanceItemChars = Math.min(CALIBRATED.acceptanceItemChars, Math.max(120, Math.floor(maxTaskChars / 10)));
  ...
  const implementCap = clamp(Math.floor(ctx * R.implementReportFraction), R.implementReportCapMin, R.implementReportCapMax);
  const implementTarget = Math.min(R.implementReportTargetTokens, Math.floor(implementCap / 2) + 64);
  const scoutCap = clamp(Math.floor(ctx * R.scoutReportFraction), R.scoutReportCapMin, R.scoutReportCapMax);
  ...
  const maxFindings = clamp(Math.floor((scoutCap - 40) / 60), R.scoutFindingsMin, R.scoutFindingsMax);
  const decomposeCap = clamp(Math.floor(ctx * R.decomposeReportFraction), R.decomposeReportCapMin, R.decomposeReportCapMax);
  ...
  let maxExcerptLinesTotal = clamp(Math.floor(ctx / 512), R.scoutExcerptLinesTotalMin, R.scoutExcerptLinesTotalMax);
  const excerptOverride = envInt(env, "NOMARMY_SCOUT_MAX_EXCERPT_LINES");
  if (excerptOverride) { maxExcerptLinesTotal = excerptOverride; overrides.push("NOMARMY_SCOUT_MAX_EXCERPT_LINES"); }

  return {
    contextPerNom: ctx,
    source: effectiveSource,
    brief: { maxTaskChars, maxAcceptanceItemChars, maxAcceptanceItems: 20 },
    report: {
 ...
 */
export function checkBrief({ task, acceptance = [] }, budgets) {
  const problems = [];
  const b = budgets.brief;
  const taskLen = String(task ?? "").length;
  if (taskLen > b.maxTaskChars) {
    problems.push(`objective is ${taskLen} characters; this nom's ${budgets.contextPerNom}-token context (${budgets.source}) allows ${b.maxTaskChars}. Split the job.`);
  }
 ...
 *   5. the v1.3 design target, labelled as an assumption
 */
...
export function describeBudgets(b) {
  return [
    `context per nom: ${b.contextPerNom} tokens (${b.source})`,
